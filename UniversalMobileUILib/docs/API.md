...
local Window = Library:CreateWindow({
    ...
    Center = true,
    AutoShow = true,
    Size = Vector2.new(430, 350),
    Position = Vector2.new(12, 54),
    TabWidth = 86,
    TabPadding = 4,
...
- `MobileOpenText`: text shown on the small top-center reopen button.

Mobile behavior:

- Windows are clamped to the current viewport.
- Left and right groupbox columns stack vertically for readability.
- The close button sits at the top middle of the window.
- Closing the window shows a small top-center `Open` button.
...
## Save Manager Autosave

Autosave is off by default. Manual save/load buttons created by `SaveManager:BuildConfigSection(...)` still work.

```lua
SaveManager:SetAutoSave(true, 2) -- save at most once every 2 seconds after changes
SaveManager:SetAutoSave(false) -- return to manual-only saves
```
