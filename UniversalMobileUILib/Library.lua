--[[
    ...
    A standalone Roblox Lua UI library with the same API as UniversalUILib,
    tuned for phones and tablets.
...
local DEFAULT_WINDOW_SIZE = Vector2.new(430, 350)
local DEFAULT_WINDOW_POSITION = Vector2.new(12, 54)
local MOBILE_MIN_WINDOW_SIZE = Vector2.new(300, 260)
local MOBILE_MARGIN = 10
local MOBILE_TOPBAR_HEIGHT = 30
local MOBILE_TAB_WIDTH = 86
local MOBILE_TAB_HEIGHT = 24
local MOBILE_BODY_GAP = 6
local INPUT_TYPES = {
...
local function safeName(value)
    return tostring(value or "UniversalMobileUILib"):gsub("[^%w_%-]", "_")
end
...
local function getViewportSize()
    ...
    return Vector2.new(430, 760)
end

local function fitWindowSize(size)
    local viewport = getViewportSize()
    local maxWidth = math.max(MOBILE_MIN_WINDOW_SIZE.X, viewport.X - (MOBILE_MARGIN * 2))
    local maxHeight = math.max(MOBILE_MIN_WINDOW_SIZE.Y, viewport.Y - 88)
    return Vector2.new(
        math.clamp(size.X, MOBILE_MIN_WINDOW_SIZE.X, maxWidth),
        math.clamp(size.Y, MOBILE_MIN_WINDOW_SIZE.Y, maxHeight)
    )
end
...
function Library:CreateScreenGui(name)
    local gui = Utility:Create("ScreenGui", {
        Name = name or "UniversalMobileUILib",
        ResetOnSpawn = false,
        IgnoreGuiInset = true,
...
function Library:GetScreenGui()
    ...
    return self:CreateScreenGui("UniversalMobileUILib")
end
...
function Library:Notify(message, duration, title)
    ...
    frame.Position = UDim2.fromOffset(300, 0)
...
local function makeGroup(parent, title)
    ...
    local layout = Utility:AddList(content, 4)
    layout.HorizontalAlignment = Enum.HorizontalAlignment.Left
    Utility:AddPadding(content, 0, 0, 0, 6)
...
local function makeColumn(parent)
    ...
    local layout = Utility:AddList(column, 6)
    layout.HorizontalAlignment = Enum.HorizontalAlignment.Left
    return column
end

function GroupMethods:AddLabel(text, wrap)
    local height = wrap and math.max(20, select(2, Utility:GetTextBounds(text, Library.Theme.TextSize, Library.Theme.Font, 220)) + 6) or 18
...
function GroupMethods:AddDivider()
    local container = createControlContainer(self.Content, 7)
...
function GroupMethods:AddToggle(index, info)
    info = info or {}
    local container = createControlContainer(self.Content, 23)
...
function GroupMethods:AddSlider(index, info)
    ...
    local container = createControlContainer(self.Content, info.Compact and 24 or 36)
    local label
    ...
    local valueLabel = Utility:TextLabel({
        ...
        Position = UDim2.new(1, -64, 0, info.Compact and 0 or 0),
        TextXAlignment = Enum.TextXAlignment.Right,
        Parent = container
    })
    local barY = info.Compact and 9 or 22
...
function GroupMethods:AddInput(index, info)
    info = info or {}
    local container = createControlContainer(self.Content, 42)
    local label = Utility:TextLabel({
        Text = info.Text or tostring(index),
...
function GroupMethods:AddDropdown(index, info)
    ...
    Utility:Connect(button.MouseButton1Click, function()
        ...
        if visible then
            ...
            popup.Size = UDim2.fromOffset(math.max(button.AbsoluteSize.X, 170), math.min(156, 8 + (#object.Values * 25)))
        end
    end)
...
function WindowMethods:AddTab(name)
    ...
    Utility:AddList(columns, 6, Enum.FillDirection.Vertical)
...
function WindowMethods:SetVisible(visible)
    ...
    if self.MobileToggleButton then
        self.MobileToggleButton.Visible = not self.Visible
    end
    return self
end
...
local function makeResizable(frame, handle, minSize)
    minSize = minSize or MOBILE_MIN_WINDOW_SIZE
    ...
    Utility:Connect(UserInputService.InputChanged, function(input)
        if resizing and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
            ...
            local viewport = getViewportSize()
            local width = math.clamp(startSize.X + delta.X, minSize.X, math.max(minSize.X, viewport.X - (MOBILE_MARGIN * 2)))
            local height = math.clamp(startSize.Y + delta.Y, minSize.Y, math.max(minSize.Y, viewport.Y - 88))
...
function Library:CreateWindow(options)
    ...
    local gui = self:GetScreenGui()
    local size = fitWindowSize(options.Size or DEFAULT_WINDOW_SIZE)
    local position = options.Position or DEFAULT_WINDOW_POSITION
    local tabWidth = options.TabWidth or MOBILE_TAB_WIDTH
    local frame = Utility:Frame({
        Name = safeName(options.Title or self.Name),
        Size = UDim2.fromOffset(size.X, size.Y),
        Position = options.Center ~= false and UDim2.fromScale(0.5, 0.5) or UDim2.fromOffset(position.X, position.Y),
        AnchorPoint = options.Center ~= false and Vector2.new(0.5, 0.5) or Vector2.new(0, 0),
    ...
    local topbar = Utility:Frame({
        Name = "Topbar",
        Size = UDim2.new(1, 0, 0, MOBILE_TOPBAR_HEIGHT),
        BackgroundColor3 = self.Theme.Main,
        Parent = frame
    })
    ...
    local body = Utility:Create("Frame", {
        BackgroundTransparency = 1,
        Position = UDim2.fromOffset(6, MOBILE_TOPBAR_HEIGHT + MOBILE_BODY_GAP),
        Size = UDim2.new(1, -12, 1, -(MOBILE_TOPBAR_HEIGHT + MOBILE_BODY_GAP + 6)),
        Parent = frame
    })
    local sidebar = Utility:Frame({
        Name = "Tabs",
        Size = UDim2.fromOffset(tabWidth, 1),
        BackgroundColor3 = self.Theme.Main,
        Parent = body
    })
    ...
    Utility:AddPadding(sidebar, 4, 4, 4, 4)
    ...
    local tabButtonLayout = Utility:AddList(tabButtons, options.TabPadding or 4)
    Utility:AutoCanvas(tabButtons, tabButtonLayout, 6)

    local pageHolder = Utility:Frame({
        Name = "Pages",
        Position = UDim2.fromOffset(tabWidth + 6, 0),
        Size = UDim2.new(1, -(tabWidth + 6), 1, 0),
        BackgroundTransparency = 1,
        Parent = body
    })
    ...
    local mobileToggle = Utility:TextButton({
        Text = options.MobileOpenText or "Open",
        AnchorPoint = Vector2.new(0.5, 0),
    ...
    Library:RegisterTheme(mobileToggle, { BackgroundColor3 = "Main", TextColor3 = "Text" })
    Utility:AddCorner(mobileToggle, 6)
    Utility:AddStroke(mobileToggle, "Accent", 1)
    window.MobileToggleButton = mobileToggle

    Utility:Connect(close.MouseButton1Click, function()
        window:SetVisible(false)
    end)
    Utility:Connect(mobileToggle.MouseButton1Click, function()
        window:SetVisible(true)
    end)
    makeDraggable(frame, topbar)
    makeResizable(frame, resize, MOBILE_MIN_WINDOW_SIZE)
